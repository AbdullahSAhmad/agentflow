...
export class AnalyticsPanel {
  ...
  constructor(store: StateStore) {
    ...
    // Listen for updates to check threshold
    this.store.on('agent:update', () => this.checkThreshold());
  }
    ...
    content.innerHTML = `
      ...
      <div class="analytics-cards">
        <div class="analytics-card total-cost">
          ...
          <div class="card-sub">${this.formatTokens(totalInput)} in / ${this.formatTokens(totalOutput)} out</div>
        </div>
        <div class="analytics-card velocity">
          <div class="card-label">Token Velocity</div>
          <div class="card-value">${this.formatTokens(Math.round(velocity))}<span class="card-unit">/min</span></div>
  ...
  private renderAgentBar(agent: AgentSnapshot, totalCost: number): string {
    ...
    return `<div class="agent-bar">
      <div class="agent-bar-label">
        ...
        <span class="agent-bar-name">${this.esc(agent.name)}</span>
  ...
  private renderZoneBars(zoneCosts: Map<ZoneId, number>, totalCost: number): string {
    ...
    return entries.map(([zoneId, cost]) => {
      ...
      const pct = totalCost > 0 ? (cost / totalCost) * 100 : 0;
      const color = zone ? '#' + zone.color.toString(16).padStart(6, '0') : '#666';
